 ...
 * Groq Model Adapter
 *
 * Uses fetch() to call the Groq OpenAI-compatible API for invoice parsing and Zoho PO mapping.
...
function getApiKey(): string | null {
    ...
    if (!key) {
        console.warn('[GroqAdapter] GROQ_API_KEY not configured');
    }
    return key || null;
}
...
/**
 * Call the Groq Chat Completions API with the given messages.
 */
async function callGroq(
    ...
    if (!response.ok) {
        ...
        console.error('[GroqAdapter] API error:', response.status, errorText);
...
export class GroqAdapter {
    ...
    async parseInvoice(documentBase64: string, filename: string): Promise<AIParseResult> {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                console.warn(`[GroqAdapter] parseInvoice attempt ${attempt + 1} failed:`, lastError.message);
        ...
        throw new Error(
            `[GroqAdapter] parseInvoice failed after ${MAX_RETRIES + 1} attempts: ${lastError?.message}`
        );
    }
    ...
    async mapToZohoPayload(
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                lastError = error instanceof Error ? error : new Error(String(error));
                console.warn(`[GroqAdapter] mapToZohoPayload attempt ${attempt + 1} failed:`, lastError.message);
        ...
        throw new Error(
            `[GroqAdapter] mapToZohoPayload failed after ${MAX_RETRIES + 1} attempts: ${lastError?.message}`
        );
    }
    ...
    private async _doParse(
        ...
        // Groq/Llama doesn't natively support image input via the chat completions endpoint,
        // so we send the base64 content as text with a note about the document.
        // For actual image support, a vision-capable model endpoint would be needed.
        const userMessage = `Parse this Proforma Invoice document (${filename}). The document content is base64-encoded below. Extract all fields and return valid JSON only.\n\nDocument (base64): ${documentBase64.substring(0, 200)}... [${documentBase64.length} chars total]`;
        ...
        try {
            ...
            throw new Error(`Failed to parse JSON from Groq response: ${content.substring(0, 200)}`);
        }
    ...
    private async _doMap(
        ...
        try {
            ...
            throw new Error(`Failed to parse JSON from Groq mapping response: ${content.substring(0, 200)}`);
        }
